...
def get_dataset(
    ...
    if large_dataset:
        Dataset = LargeGeoSSlPretrainDataset
...
@registry.register_trainer("geossl_pretrain")
class GeoSSLPretrainer(BaseTrainer):
    def __init__(
            self,
            model: BaseModel,
            dataset: Dataset,
    ...
    def from_config(cls, config):
        ...
        model = cls._load_model(config["model"], config["dataset"]["preprocess_params"], dataset, local_world_size,
                                rank)
        NCSN_model_01 = \
            NCSN_version_03(128, sigma_begin=10, sigma_end=0.01, num_noise_level=30, anneal_power=0.05).to(rank)
        NCSN_model_02 = \
            NCSN_version_03(128, sigma_begin=10, sigma_end=0.01, num_noise_level=30, anneal_power=0.05).to(rank)
        ...
        loss1 = cls._load_loss(config["optim"]["loss"])
        loss2 = cls._load_loss(config["optim"]["loss"])
    ...
    def _load_dataloader(optim_config, dataset_config, dataset, sampler):
        ...
        if isinstance(dataset_config["src"], dict):
            ...
            val_loader = get_dataloader(val_dataset, batch_size=batch_size, sampler=sampler) if val_ratio != 0 else None
            test_loader = get_dataloader(
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            self.metrics = accum_loss / len(self.train_loader)
    ...
    def validate(self, split="val"):
        ...
        for i in range(0, len(loader_iter)):
            with torch.no_grad():
                ...
                accum_los += loss.detach().item()

        return accum_los / len(loader_iter)
    ...
    def predict(self, loader, split):
        ...
        predict_loss = self._metrics_predict[type(self.loss_fn1).__name__]["metric"]
